"""
ماژول Scorer — ترکیب وزنی شاخص‌های نرمال‌شده برای رسیدن به امتیاز نهایی.

منطق کلی:
  ۱. برای هر مؤلفه (Experience/Expertise/Authority/Trust)، میانگین وزنی
     شاخص‌های usable آن مؤلفه محاسبه می‌شود (شاخص‌های missing/excluded
     طبق خروجی Normalizer کنار گذاشته می‌شوند).
  ۲. امتیاز نهایی = ترکیب وزنی ۴ امتیاز مؤلفه، با وزن‌های خوانده‌شده
     از فایل config (weights_equal.yaml یا weights_ahp.yaml).
